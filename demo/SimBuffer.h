...
struct SimBuffers
{
	...
	void BuildConstraints() {
		if (rigidOffsets.size()) {
			...
			CalculateRigidLocalPositions(&positions[0], positions.size(), &rigidOffsets[0], &rigidIndices[0], numRigids, &rigidLocalPositions[0]);
	...
	void SendBuffers(NvFlexSolver *flex) {
		...
		NvFlexSetParticles(flex, positions.buffer, numParticles);
		NvFlexSetVelocities(flex, velocities.buffer, numParticles);
		NvFlexSetNormals(flex, normals.buffer, numParticles);
		NvFlexSetPhases(flex, phases.buffer, phases.size());
		NvFlexSetRestParticles(flex, restPositions.buffer, restPositions.size());

		NvFlexSetActive(flex, activeIndices.buffer, numParticles);
		...
		if (springIndices.size()) {
			...
			NvFlexSetSprings(flex, springIndices.buffer, springLengths.buffer, springStiffness.buffer, springLengths.size());
		}

		// rigids
		if (rigidOffsets.size()) {
			NvFlexSetRigids(flex,
				rigidOffsets.buffer,
				rigidIndices.buffer,
				rigidLocalPositions.buffer,
				rigidLocalNormals.buffer,
				rigidCoefficients.buffer,
				rigidRotations.buffer,
				rigidTranslations.buffer,
				rigidOffsets.size() - 1,
				rigidIndices.size());
		}
		...
		if (inflatableTriOffsets.size()) {
			NvFlexSetInflatables(flex,
				inflatableTriOffsets.buffer,
				inflatableTriCounts.buffer,
				inflatableVolumes.buffer,
				inflatablePressures.buffer,
				inflatableCoefficients.buffer,
				inflatableTriOffsets.size());
		}

		// dynamic triangles 
		if (triangles.size()) {
			NvFlexSetDynamicTriangles(flex, triangles.buffer, triangleNormals.buffer, triangles.size() / 3);
		}
		...
		if (shapeFlags.size()) {
			NvFlexSetShapes(
				flex,
				shapeGeometry.buffer,
				shapePositions.buffer,
				shapeRotations.buffer,
				shapePrevPositions.buffer,
				shapePrevRotations.buffer,
				shapeFlags.buffer,
				int(shapeFlags.size()));
		}
	}
