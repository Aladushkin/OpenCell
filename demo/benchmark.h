...
void BenchmarkUpdate()
{
	...
	if (g_frame >= 200 && g_frame < 400)
	{
		...
		for (int i = 0; i < g_numDetailTimers; i++) {
			sDTimersSum[i].name =  flexController.GetDetailTimers()[i].name;
			sDTimersSum[i].time += flexController.GetDetailTimers()[i].time;
		}

		sTimersSum.total += flexController.GetTimers().total;
...
{
	g_benchmarkFile.open(g_benchmarkFilename, std::ofstream::out | std::ofstream::app);
	g_benchmarkFile << "Compute Device: " << flexController.GetDeviceName() << std::endl;
